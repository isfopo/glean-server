/**
 * Basic usage example for the Glean ATproto Server
 *
...
async function runExample() {
  try {
    console.log("🚀 Starting Glean ATproto Server Example\n");
    ...
    const accountData = await makeRequest("/api/auth/createAccount", {
      ...
      body: JSON.stringify({
        ...
        profile: {
          ...
          description: "A demonstration user for the Glean platform",
        },
      }),
    });
    ...
    const itemForm = await createFormData(
      {
        ...
        description:
          "A beautiful photo of the Golden Gate Bridge taken from Crissy Field",
      },
      {
    ...
    console.log(
      `   Location: ${itemData.geomarker.lat}, ${itemData.geomarker.lng}`,
    );
    ...
    console.log(
      `   Display Name: ${userProfile.profile?.displayName || "Not set"}`,
    );
